/*document.addEventListener("DOMContentLoaded", function () {
    ...
    btnAvancar.addEventListener("click", async function () {
        ...
        if (tipo === "aluno") {
            ...
            try {
                const response = await fetch('http://localhost:3000/api/plano'); // Use o endereço do backend
                const planos = await response.json();

                // Adicionar título "Escolher Plano"
                const tituloEscolherPlano = document.createElement('h2');
                tituloEscolherPlano.textContent = "Escolher um Plano";

                // Garantir que o título não seja apagado ao renderizar os planos
                planosContainer.innerHTML = ''; // Limpa qualquer conteúdo antigo
                planosContainer.appendChild(tituloEscolherPlano); // Adiciona o título
                ...
                planos.forEach(plano => {
                    ...
                    checkbox.value = plano.valor; // Valor do plano
                ...
                // Garantir seleção única de plano
                setTimeout(() => {
    ...
    cartaoCheckbox.addEventListener("change", function () {
        if (cartaoCheckbox.checked) {
            desmarcarOutros(cartaoCheckbox);
    ...
    boletoCheckbox.addEventListener("change", function () {
        if (boletoCheckbox.checked) {
            desmarcarOutros(boletoCheckbox);
    ...
    // Adicionar lógica para cálculo de parcelas
    const parcelasSelect = document.getElementById("parcelas");
    const valorParcela = document.createElement("p"); // Exibir o valor por parcela

    // Adicionar o elemento de exibição do valor por parcela
    dadosCartao.appendChild(valorParcela);

    // Atualizar o valor por parcela ao selecionar um plano
    planosContainer.addEventListener("change", function (event) {
        if (event.target.name === "plano") {
            const planoValor = parseFloat(event.target.value); // Valor do plano
            atualizarValorParcela(planoValor);
        }
    });

    // Atualizar o valor por parcela ao mudar o número de parcelas
    ...
    // Função para calcular e exibir o valor por parcela
    function atualizarValorParcela(valorPlano) {
        const parcelas = parseInt(parcelasSelect.value);
        const valorPorParcela = valorPlano / parcelas;
        valorParcela.textContent = `Valor por parcela: R$ ${valorPorParcela.toFixed(2)}`;
    }
});


document.addEventListener("DOMContentLoaded", function () {
    const btnConcluir = document.getElementById("btn_concluir"); // Botão "Concluir"
    const planosContainer = document.getElementById("planos-container");
    const cartaoCheckbox = document.getElementById("cartao_credito");
    const boletoCheckbox = document.getElementById("boleto");
    const pixCheckbox = document.getElementById("pix");
    ...
    btnConcluir.addEventListener("click", async function () {
        ...
        const planoId = parseInt(planoSelecionado.id.replace("plano_", "")); // Extrair o ID do plano
        ...
        const dados = {
            planoId: planoId, // ID do plano selecionado
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(errorData.error || "Erro ao salvar os dados.");
            }
        } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnConcluir.addEventListener("click", async function () {
        ...
        const senha = document.getElementById("password_cadastro").value;
         ...
         // Capturar o valor do tipo de cadastro
        ...
        if (document.getElementById("cartao_credito").checked) {
            formaPagamento = "CartaoCredito";
        ...
        const dados = {
            ...
            planoId,
            formaPagamento,
            dadosCartao, // Só será enviado se for Cartão de Crédito
        };
   ...
   /* const dados = {
        ...
        senha: "senha123",
        tipo: "aluno", // Deve corresponder ao enum
        cpf: "12345678901",
        planoId: 1, // Certifique-se de que o plano com ID 1 existe
    };*/
